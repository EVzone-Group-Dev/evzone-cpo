...
- `../ocpi-gateway/src/modules/sessions/sessions.service.ts`
- `../ocpi-gateway/src/modules/cdrs/cdrs.service.ts`
- `../ocpi-gateway/src/modules/tariffs/tariffs.service.ts`
- `../ocpi-gateway/src/modules/locations/locations.service.ts`
- `../ocpi-gateway/src/modules/tokens/tokens.service.ts`
...
- `../ocpi-gateway/src/modules/partners/partners.service.ts`
- `../ocpi-gateway/src/modules/partners/partner-observability.service.ts`
- `../ocpi-gateway/src/modules/partners/partner-observability.consumer.ts`
- `../ocpi-gateway/scripts/ocpi-event-publisher-selftest.ts`
- `../ocpi-gateway/scripts/partner-observability-selftest.ts`
- `../ocpi-gateway/scripts/partner-lifecycle-events-selftest.ts`
...
The main caution here is completeness, not legitimacy. The code is useful and materially ahead of a mock-only implementation, the health endpoint is now dependency-aware, the command path now includes reservation handling through the backend into the CPMS command queue, the backend worker now retries transient OCPI callbacks while persisting callback delivery metadata for tracing, `ocpi-gateway` now emits real Kafka events for commands, charging profiles, sessions, CDRs, tariffs, locations, tokens, credentials, and partner lifecycle changes, and the admin surface now includes Redis-backed partner observability summaries built from that broader traffic and surfaced directly in the CPO UI. But the gateway still depends heavily on backend `/internal/ocpi/*` contracts, Kafka event coverage is still partial rather than truly universal across every OCPI surface, and there is still no evidence of OCPI `2.3.0` support or formal conformance automation yet.
...
| Roaming and interoperability (OCPI) | OCPI partners, commands, CDRs, roaming settlements, hub connectivity | Partial | `src/pages/roaming/OCPIPartnersPage.tsx`, `src/pages/roaming/RoamingSessionsPage.tsx`, `src/pages/roaming/OCPICommandsPage.tsx`, `src/pages/roaming/OCPICDRsPage.tsx`, `../ocpi-gateway/src/modules/ocpi/ocpi.service.ts`, `../ocpi-gateway/src/modules/commands/commands.service.ts`, `../ocpi-gateway/src/modules/sessions/sessions.service.ts`, `../ocpi-gateway/src/modules/cdrs/cdrs.service.ts`, `../ocpi-gateway/src/modules/tariffs/tariffs.service.ts`, `../ocpi-gateway/src/modules/locations/locations.service.ts`, `../ocpi-gateway/src/modules/tokens/tokens.service.ts`, `../ocpi-gateway/src/modules/partners/partners.service.ts`, `../ocpi-gateway/src/modules/ocpi/core/ocpi-event-publisher.service.ts`, `../ocpi-gateway/src/modules/partners/partner-observability.service.ts`, `../evzone-backend/apps/api/src/modules/ocpi-internal/ocpi-internal.controller.ts`, `../evzone-backend/apps/worker/src/modules/commands/ocpi-command-callback.service.ts`, `../ocpi-gateway/docs/internal-ocpi-contracts.md` | This is no longer mock-only. EVzone has a real OCPI gateway with working internal command relay, broader Kafka event coverage across commands, charging profiles, sessions, CDRs, tariffs, locations, tokens, credentials, and partner lifecycle changes, retrying backend callback delivery, and partner observability summaries now surfaced across the CPO UI in partners, sessions, commands, and CDR workflows, but it is still backend-dependent, limited to `2.2.1` and `2.1.1`, and not yet proven as a fully hardened roaming platform. |
...
- real Kafka event emission across commands, charging profiles, sessions, CDRs, tariffs, locations, tokens, credentials, and partner lifecycle changes
- partner observability summaries and recent-event monitoring across those roaming modules and handshake flows
- CPO UI surfacing for partner observability, callback health, retry backlog, module coverage, and roaming-flow impact across sessions, commands, and CDR operations
...
- Kafka coverage is broader now, but still not yet universal across the full OCPI surface such as error events, hub-client-info flows, and formal conformance lifecycle instrumentation
...
This is the biggest change from the earlier review. EVzone already has a strong OCPP gateway and a partial OCPI gateway, and the latest phases added broader outbound OCPP command depth, a real reservation command path from OCPI through the backend into OCPP, much broader OCPI event coverage, and visible partner observability inside the CPO UI across roaming data flows, token handling, credentials and partner lifecycle changes, and the downstream operator views for sessions, commands, and CDRs. The next step is to finish depth, conformance, and operational hardening.
...
The platform already has a real OCPP gateway and a partial OCPI gateway, and the current codebase now goes a step further by supporting reservation commands end-to-end across the OCPI, backend, and OCPP path while also emitting and observing a much broader OCPI operational event stream that now reaches tokens, credentials, and partner lifecycle changes too. The main gap is no longer protocol existence. The main gap is protocol completeness, roaming depth, productized reservation and payment flows, advanced auth productization, and broader energy orchestration.
